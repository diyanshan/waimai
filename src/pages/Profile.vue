<template>
  <section class="profile">
    ...
    <section class="profile-number" @click="togo">
    ...
    <section class="profile_my_order border-1px" v-show="user._id">
      <mt-button type="default" style="width: 100%"@click.native="logout">退出登录</mt-button>
    </section>
  </section>
</template>
...
<script>
  ...
  export default {
    ...
    methods:{
      togo(){
        this.$router.push(this.user._id ? '/user_info' : '/login')
      },
      logout () {
        if(confirm('你确认退出吗?')) {
          // 请求后台退出
